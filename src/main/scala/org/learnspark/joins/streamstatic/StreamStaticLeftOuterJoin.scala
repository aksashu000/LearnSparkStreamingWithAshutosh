...
object StreamStaticLeftOuterJoin {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      .builder
      .master("local[*]")
      .appName("Stream-Static Left Outer Join Example")
      .getOrCreate()
    ...
    val streamDeviceInfoSchema = StructType(List(
      StructField("id", StringType, nullable = true),
    ...
    val deviceInfoStreamingDataset: Dataset[DeviceInfo] =
      streamingDf
        ...
        .select(from_json($"jsonValue", schema = streamDeviceInfoSchema).as("data"))
        .select("data.*")
        .as[DeviceInfo]
